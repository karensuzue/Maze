...
class ToPNG():
    ...
    def render(self):
        ...
        # Draw borders:
        draw.line((0, 0, 0, self.cell_size * self.grid.rows - 1),
                  fill=self.wall_color, width=1)  # Left
        draw.line((self.cell_size * self.grid.cols - 1, 0,
                   self.cell_size * self.grid.cols - 1,
                   self.cell_size * self.grid.rows - 1), fill=self.wall_color,
                  width=1)  # Right
        draw.line((0, 0, self.cell_size * self.grid.cols - 1, 0),
                  fill=self.wall_color, width=1)  # Top
        draw.line((0, self.cell_size * self.grid.rows - 1,
                   self.cell_size * self.grid.cols - 1,
                   self.cell_size * self.grid.rows - 1), fill=self.wall_color,
                  width=1)
